...
void update_no(int ind,int i=1){
	node& a=segTree[i]; int m=(a.l+a.r)/2;
	if(a.l==a.r && a.l==ind) { a.reset(ind,ind,val[ind]); return; }
	else update_no(ind,2*i+(ind>m));
	merge(i);
}
/*
void push(int i)
...
int main()
{
	...
	{
		cin>>t>>x>>y;
		if(t)	cout<<(act_query(x,y))<<endl;
		else { val[x]=y; update_no(x); }
	}
} 
